/*************************************************************************
 ...
 hv4d - hypervolume indicator computation in 4D
...
/*************************************************************************
 ...
 ---------------------------------------------------------------------
 [1] Andreia P. Guerreiro, Carlos M. Fonseca, and Michael T. M. Emmerich.
    A fast dimension-sweep algorithm for the hypervolume indicator in four
    dimensions. In CCCG, pages 77–82, 2012.

 [2] Andreia P. Guerreiro. Efficient algorithms for the assessment
...
static double
hv4d_increment3DA(dlnode_t *p, dlnode_t * ynext, dlnode_t *znext){
...
static double
hv4d_increment3D(dlnode_t *list, dlnode_t *p, const double * ref){
    double loss = 0, gain;
    ...
    while(q != head3){
        ...
        if(q->x[2] >= p->x[2] && q->x[1] >= p->x[1] && q->x[0] >= p->x[0]){
            remove_y(q);
            remove_z(q);
            loss += hv4d_increment3DA(q, q->nexty, q->nextz);
    ...
    gain = hv4d_increment3DA(p, yprev->nexty, zprev->nextz);
